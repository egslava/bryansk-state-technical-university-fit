...
|----------+----------+----------+----------+----------+----------+----------|
PROBLEM_SITUATION
yield sprintf("|  %-8d|  %-8.2f|  %-8s|  %-8.2f|  %-8.2f|  %-8.2f|  %-8.2f|\n", n, $Cn, dust, $H, $D, $V, $Tr);
yield "'----------'----------'----------'----------'----------'----------'----------'\n"

# формула №7
$step += 1;
@m7 = (($Cn * $V) / (3600.0 * (10.0 ** 3.0))).round(PRECISE);
...
$step += 1;
@m9 = 	( (1.0) / 
		  (0.67 +  0.1 * (f ** 0.5) + 0.34 * (f ** (1.0/3.0) ))).round(PRECISE);
yield <<COMMENT
...
def n (_Vm)
  ...
  yield "                                             М          \n"
    ...
    yield "           М                  \n"
		...
		result = (0.582 * (_Vm ** 2.0) - 2.13 * _Vm + 3.13).round(PRECISE);
		yield "   n = #{result}\n"
		result
	else
...
$PDV_warm = ( $PDKmr * ( ($H ** 2.0) * (($V_1 * $DT) ** (1.0/3.0)) )/
...
yield <<RESULT
Выпишем полученные результаты в таблицу:
.----------.----------.----------.----------.----------.----------.----------.----------.
|Выбросы   |    C     |   C      | ПДК      |    M,    |   ПДВ,   |     Z,   |     X    |
|          |     п    |    М.Р.  |    Р.З.  |          |          |          |      М   |
|          |        3 |      3   |      3   |          |          |          |          |
|          |     мг/м |  мг/м    |  мг/м    |   г/с    |   г/с    |     %    |     M    |
|----------|----------|----------|----------|----------|----------|----------|----------|
RESULT
yield (sprintf "|Нагретые  |%-10.2f|%-10.2f|%-10.2f|%-10.2f|%-10.2f|%-10.2f|%-10.2f|\n", $Cn, $Cmr_warm_result, $PDKrz, @m7, $PDV_warm, $Z_warm, $Xm_warm)
yield "|----------|----------|----------|----------|----------|----------|----------|----------|\n"
yield (sprintf "|Холодные  |%-10.2f|%-10.2f|%-10.2f|%-10.2f|%-10.2f|%-10.2f|%-10.2f|\n", $Cn, $Cmr_cold, $PDKrz, @m7, $PDV_cold, $Z_cold, $Xm_cold)
yield "'----------'----------'----------'----------'----------'----------'----------'----------'\n"
end